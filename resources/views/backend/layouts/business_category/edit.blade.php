...
@section('content')
    ...
    <div class="card">
        <div class="card-body p-4">
            <form
                action="{{ isset($category) ? route('backend.business.category.update', ['id' => $category->id]) : route('backend.business.category.store') }}"
                method="POST"
                class="space-y-4">
                @csrf
                @if (isset($category))
                ...
                <div>
                    <button type="submit"
                        class="bg-indigo-600 btn btn-primary text-white px-4 py-2 rounded-md hover:bg-indigo-700 transition">
